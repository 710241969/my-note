<template>
  <div>
    ...
    <!-- 文件 -->
    <div v-else class="file-div" :class="[isClick?'click-file-div':'no-click-file-div']" :style="{paddingLeft:`${left}px`}" @click="clickFile(fileTree)" @dblclick="dbClickOpenFile(fileTree)">
...
export default {
  ...
  computed: {
    ...
    inputVisible() {
      return (
        this.$store.getters.currentDirPath === this.fileTree.path &&
        this.$store.getters.inputVisible &&
        this.fileTree.isDir
      )
    },
    createType() {
  ...
  methods: {
    ...
    dbClickOpenFile(fileObj) {
      ...
      file.content = fileOperator.readFile(fileObj.path) // 获取文件内容
      this.$store.dispatch(types.ADD_FILE_TAB_ITEM, file) // 文件BAR条添加文件对象
    ...
    createFileOrDir() {
      ...
      return new Promise((resolve, reject) => {
        if (this.inputName) {
          fileOperator
            .createFileOrDir(
              this.$store.getters.currentDirPath + this.inputName,
              this.$store.getters.getCreateType
            )
            .then(() => {
              this.inputName = ''
              this.fileTree = fileOperator.throughPath(this.fileTree) // 重新生成文件树
              return resolve()
            })
        } else {
          this.inputName = ''
